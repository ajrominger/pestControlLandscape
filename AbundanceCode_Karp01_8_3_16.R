######################
# Define study names # START CUSTOMIZATION
######################
setwd("~/Dropbox/Research/pestControlLandscape") # change to your working drive. this is where results will saved. 
...
response_path <- '~/Dropbox/Research/pestControlLandscape' # Change to where the spreadsheet with the data is located. Remember to include the forward slash at the end. 
landscape_name <- "V1_Karp01_AllC_Gaussian_3_decays_weighted_cover_percent.csv" # Change to name of the landscape file.
landscape_path <- '~/Dropbox/Research/pestControlLandscape' # Change to where the land use spreadsheet is located. Remember to include the forward slash at the end. 
...
###########################################################
# Make decisions about which functional groups to analyze # START CUSTOMIZATION
###########################################################
# Subset to functional group of interest #
...
analysis_type= 'dominant pest' #"all_enemies" #Fill in the analysis type that you will conduct. If a particular pest, then write out the name of the pest. Only choose to analyze a particular dominant pest if it appears above in the "sp_to_analyze" table 
functional_group <- 'dominant pest' # c("predator","parasitoid") # Choose the name(s) of the study functional group(s) that your analysis will focus on. May be 1 or more functional groups but names must match the names in the spreadsheet. If you will focus on one dominant pest species, still put in dominant pest. 
species<-"nasonovia ribisnigri" # If doing an analysis of a particular dominant pest, fill this in otherwise do not fill this in. 

df1=df0[df0$Functional_Group%in%functional_group,] # Use this to subset down the broad functional group of interest.
if(species!=""){df1=df1[df1$Species%in%species,]}

View(df1) # Views df1. Look at all the values written in the abundance column and try to decide whether the authors reported the number of insects trapped across surveys or a mean abundance across surveys. A mean abundance would usually be indicated by decimal numbers in the abundance column. 
...
###########
#Check unknown sites (unknown if sampled or not sampled)#  
###########
notINCLUDED=df2[is.na(match(df2$Site,siteTAB$Site)),1]; print(notINCLUDED) # If sites show up here, they were in the site tab but not the data tab. We don´t know if they were sampled and had zero data, or if they weren´t sampled, or if the data are just missing in the file --> recording how many sites this concerns if any.
UnknownSites=array(dim=c(1,2));colnames(UnknownSites)=c("SitesUnknown","TotalSites")
UnknownSites[,1]=nrow(notINCLUDED); UnknownSites[,2]=length(unique(siteTAB$Site)); print(UnknownSites)
...
landuseclasses=c(1,2,3,4,5) # Define which land use classes are present and will be analyzed. Numbers are the class numbers. 
scale="250" # the spatial scale that will be analyzed
...
# Set up Model #
################
# COLLINEARITY : If collinearity was present (above), Models will be run twice. The first run (full model) will have all predictors. The second run (subset) should be specified to only have the set of predictors that do not exhibit collinearity. 
...
response="Mean_FourthStand_NPerEffort";print(df6[,response]) # Choose which response to include. If want a transformation for normality (see below for model checks), then choose either "Mean_LogStand_NPerEffort", "Mean_SqrtStand_NPerEffort", or "Mean_FourthStand_NPerEffort," otherwise use "Mean_Stand_NPerEffort." Note: Log transformation cannot be used if there were 0s present. Will appear as "NaN" if this is the case. 
covariates="~0+Sampling_Method+" # Add in any other covariates here, be sure to leave + at end of line and ~ in the beginning. Land use classes added in next line. Be sure to keep in 0 if analyzing sampling method or year is included. 
covariates="~0+"
...
# M_full <- gls(fixed=formula(paste(response,covariates,LUC), # Type of model used if there ARE NO random effects
#           method="ML",
#           correlation=corExp(form=~X_up+Y_up|Farm),
#           data=df6)
aic=AIC(M_full)
...
# IF COLLINEARITY WAS PRESENT NEED TO REPEAT FOR COLLINEAR CASE. DO NOT RUN THIS BLOCK OF CODE IF NO COLLINEARITY. 
...
M_subset <- lme(
              ...
              method="ML",
              data=df6)
# M_subset <- gls(fixed=formula(paste(response,covariates,SUB_LUC), # Type of model used if there ARE NO random effects
#           method="ML",
...
# Check residuals also of subsetted model if collinearity was present
qqnorm(resid(M_subset)); plot(resid(M_subset)~fitted(M_subset))  
Sub_Trans= "log" # Fill in either "none","sqrt", "fourth", or "log" if you transformed the data.
Sub_Normal= "fourth" # Fill in either "normal" or "not normal" depending on if residuals are normal in this model
Sub_Hetero= "none" # Fill in either "none" or "patterned residuals" if there are patterns in the residuals 
############## END CUSTOMIZATION
...
if(collinearity==TRUE){
  StudyAtr=cbind(study,country,region,crops,b_response,fun_group,species,scale, "FALSE", Sub_Trans,Sub_Normal,Sub_Hetero,Sub_AIC,obs.exp(sub_sum))
  LongStudyAtr=StudyAtr
...
###################### START CUSTOMIZATION
#CompiledResults=ModResults # Call this ONLY the first time you run a model. Otherwise, it will write over all previous analyses that you conducted. 
CompiledResults=rbind(CompiledResults,ModResults) # Call this instead of the above line for every subsequent model you run. It will append it to the bottom so that you have saved all the work. 
View(CompiledResults)
###################### END CUSTOMIZATION
